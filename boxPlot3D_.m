function boxPlot3D_(xx,g1,g2,quantDistribution, box_factor, az, el, xLabels, yLabels)
...
if nargin <1
    ...
    if length(unique(g2)) ~= length(yLabels) || length(unique(g1)) ~= length(xLabels)
        error('The number of group and that of labels donnot match!');
    end
    ...
    if ~exist('g1','var')
        ...
        width_g1 = min(diff(cases_g1)) * box_factor;
        width_g2 = min(diff(cases_g2)) * box_factor;
...
switch lenZStats
    ...
    case 5
        ...
        % delta=0.05*(currentPositions(3)-currentPositions(1));
        % delta = 0.05;
        if currentPositions(3)-currentPositions(1) == 0
            delta = 0;
        else
            delta = 0.05;
        end
